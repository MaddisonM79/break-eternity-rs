//! Compile-time constants and lookup tables for the crate.
...
/// Number of entries in [`POWERS_OF_TEN`]: `10^-323` through `10^308` inclusive.
const POWERS_OF_TEN_LEN: usize = (NUMBER_EXP_MAX - NUMBER_EXP_MIN) as usize;

/// Correctly rounded powers of ten from `10^(NUMBER_EXP_MIN+1)` through `10^NUMBER_EXP_MAX`.
///
/// Written as literals so the compiler's decimal-to-float conversion produces the
/// same values `"1e{n}".parse()` would, without needing `alloc` or a lazy cell.
#[allow(clippy::excessive_precision)]
static POWERS_OF_TEN: [f64; POWERS_OF_TEN_LEN] = [
...
/// Whether commas in string representations are ignored (`"1,000"` parses as `1000`).
pub(crate) const IGNORE_COMMAS: bool = true;

/// Whether commas are decimal points. Mutually exclusive with [`IGNORE_COMMAS`].
pub(crate) const COMMAS_ARE_DECIMAL_POINTS: bool = false;

// ---------------------------------------------------------------------------
// Helpers that only need constants (no Decimal dependency)
// ---------------------------------------------------------------------------

/// Returns `10^exp` for `exp` in `[-323, 308]` from the precomputed table.
pub(crate) fn power_of_10(exp: i32) -> f64 {
    POWERS_OF_TEN[(exp - NUMBER_EXP_MIN - 1) as usize]
}
...
mod tests {
    ...
    #[test]
    fn power_of_10_matches_parsed_literals() {
        for exp in (NUMBER_EXP_MIN + 1)..=NUMBER_EXP_MAX {
            let parsed: f64 = std::format!("1e{exp}").parse().unwrap();
            assert_eq!(power_of_10(exp), parsed, "10^{exp}");
        }
    }
